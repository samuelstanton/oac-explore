...
class PytorchBNN(Module):
    ...
    def predict(self, inputs, **kwargs):
        ...
        # randomly select component predictions
        component_idx = torch.randint(
            low=0,
            high=len(self.nn_components),
            size=torch.Size([n])
        )
        pred_locs = torch.stack([dist.mean for dist in pred_dists])
        pred_scales = torch.stack([dist.scale for dist in pred_dists])
        pred_locs = pred_locs[component_idx, ..., range(n)]
        pred_locs = pred_locs.permute(*range(1, len(self.label_shape) + 1), 0)
        pred_scales = pred_scales[component_idx, ..., range(n)]
        pred_scales = pred_scales.permute(*range(1, len(self.label_shape) + 1), 0)
        res = Normal(loc=pred_locs, scale=pred_scales)
        return res
    ...
    def fit(
        ...
        if normalize:
            ...
            self.label_mean = train_y.mean(0)
            self.label_std = train_y.std(0)
    ...
    def _save_best(self, epoch, holdout_losses, best_weights):
        ...
        for i in range(len(holdout_losses)):
            ...
            improvement = (best - current) / max(abs(best), 1.)
            if improvement > 0.01:
